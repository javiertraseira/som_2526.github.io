...
```note
💡 Un **usuario** es una persona identificable que puede iniciar sesión y usar un SO.
```

```note
💡 Un **grupo** es un conjunto de usuarios al que se le asignan las mismas reglas.
```

En los sistemas Windows de escritorio (Windows 10 / 11) se gestionan **usuarios y grupos locales**, es decir, que solo existen y funcionan dentro de ese equipo.

En **Windows Server**, además, se pueden gestionar usuarios y grupos de **dominio**, que permiten iniciar sesión y trabajar en red desde distintos equipos. Esto se verá más adelante.

Para poder usar un equipo, un usuario debe validarse (iniciar sesión) en el sistema. Dicho usuario debe existir previamente, y solo puede ser creado por un usuario con privilegios de **administrador**.
...
En Windows existen **dos tipos principales** de cuentas locales:

**Usuario estándar**

- Puede usar programas ya instalados.
...
- Es el tipo de cuenta recomendado para el **uso diario**.

**Administrador**

- Tiene **control total** del equipo.
- Puede **instalar** software y hardware.
...
El SID funciona como un DNI interno del usuario. Windows no identifica a los usuarios por su nombre, sino por su SID.
Aunque cambiemos el nombre de un usuario, su SID no cambia.
...
- Se asignan permisos al grupo.
- Los usuarios heredan esos permisos automáticamente.

Es mejor dar permisos a **grupos** que a usuarios individuales.
...
1.  **Gestión de cuentas de usuario** desde *Herramientas Administrativas \> Administración de equipos.*
2.  Desde la **consola de comandos** (CMD)

> No administrar cuentas desde la configuración de Windows o el panel de control.
...
El **UAC** (*User Account Control*) o **control de cuentas de usuario** es una característica de seguridad que se encarga de notificar alertas de seguridad del sistema al usuario. 

Lanza mensajes de alerta cuando se quiere realizar alguna acción que influya en el sistema como, por ejemplo:
- La instalación de determinados programas
- La modificación del registro de Windows
- La creación de servicios que requieren privilegios.
...
Para ello desde *Cuentas de usuario* del *Panel de Control* hacer clic en cambiar configuración de Control de cuentas de usuario. 
...
Características de dichos permisos:
