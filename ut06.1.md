...
```note
La **interfaz gráfica** de Windows se conoce como el **Shell**.
```

El shell es la interfaz gráfica de usuario principal y de Windows. Es el contenedor dentro del cual toda la interfaz de usuario se presenta, incluyendo la barra de tareas, el escritorio, el explorador de Windows, así como muchos de los cuadros de diálogo y controles de interfaz.
...
3.  El panel de control (obsoleto)
...
El comando *DISM*, el cual es la abreviatura del término Deployment Image & Servicing Management, es otra herramienta para intentar reparar archivos corruptos que impiden que Windows u otros programas funcionen como deberían.

Esta herramienta dispone de varios parámetros para realizar la reparación, como es el CheckHealth, ScanHealth y RestoreHealth, los cuales se ejecutan en este orden e intentan corregir los problemas y reparar archivos dañados.
...
> ⚠ La defragmentación **no** es posible en discos de estado sólido (SSD) o memorias USB.
...
En el menú Inicio selecciona **Configuración** y seleccionar **Aplicaciones** de la configuración de Windows.
...
A través del **Panel de control** (cada vez más obsoleto y en desuso) se puede agregar o quitar software y elementos de Windows.
...
## Comandos listado de software

Para ver el listado de software instalado se puede utilizar el comando WMIC de la siguiente forma:

    WMIC PRODUCT GET NAME,VERSION

Para ver las asociaciones de ficheros se puede usar el comando *ASSOC*:
...
Otra forma de lanzar cualquier programa es mediante el comando START [programa]
...
Para ver el listado de drivers del sistema utilizaremos el comando *driverquery* en CMD.

### msconfig

Configuración del sistema, conocido como **msconfig,** es otra sencilla herramienta de información que permitirte arreglar problemas relacionados con el inicio y arranque de Windows, y que también permite deshabilitar programas al inicio de Windows, sin necesidad de editar el Registro de Windows.
