...
Para **empaquetar** una carpeta (o directorio) con tar: 
    ...
    tar -cvf nombre_archivo.tar nombre_directorio
    tar –cvf ficherospacked.tar /home/javier/images

Para **desempaquetar** un archivo con tar:

    tar -xvf mi_archivo.tar
    tar –xvf ficherospacked.tar

Para ver el contenido de un tar:

    tar –tvf ficherospacked.tar

Se puede utilizar los parámetros *--append* o *--delete* para agregar o quitar ficheros:
...
Para comprimir con tar:
    ...
    tar -czvf empaquetado.tar.gz \*.md

Y para descomprimir:

    tar –xzvf empaquetado.tar.gz \*.md
